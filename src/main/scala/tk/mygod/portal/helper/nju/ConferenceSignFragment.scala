...
object ConferenceSignFragment {
  ...
  def openSignConnection(file: String, data: String): JValue = {
  ...
  def valueToString(value: Any): String = if (value == null) null else value.toString
  def checkKeyValue(key: String, valueRaw: Any, expected: String): AnyVal = {
