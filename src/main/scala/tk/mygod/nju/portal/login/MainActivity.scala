...
final class MainActivity extends ToolbarActivity {
  ...
  private def testNetworkMonitor(requested: Boolean = false) = App.instance.systemNetworkMonitorAvailable match {
    ...
    case 3 => if (requested) new AlertDialog.Builder(this).setTitle("Uninstall this app?")
  ...
  private def testBindedConnections(requested: Boolean = false) = App.instance.bindedConnectionsAvailable match {
    ...
    case 2 => if (requested) manageWriteSettings()
  }
}
